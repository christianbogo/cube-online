...
const importStatement = "import { detectOutliers } from './utils/analysis';\n";
let newContent = content;
if (!newContent.includes('detectOutliers')) {
    newContent = newContent.replace("import * as admin from 'firebase-admin';", "import * as admin from 'firebase-admin';\n" + importStatement);
}

const getChartDataRegex = /export const getChartData = functions\.https\.onCall\([\s\S]*?return \{ chartData, boxPlotStats \};\n\}\);/m;

const replacement = `export const getChartData = functions.https.onCall(async (data: any, context: any) => {
    ...
    const anomalies = allSolves.filter(s => {
...
newContent = newContent.replace(getChartDataRegex, replacement);

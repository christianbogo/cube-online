...
export function detectOutliers(currentSolve: number | Solve, recentSolves: Solve[] | number[], isGlobalPB: boolean = false): OutlierResult {
    ...
    if (typeof currentSolve === 'object' && currentSolve !== null) {
        ...
        currentSolveTime = currentSolve as number;
    }
    ...
    if (typeof recentSolves[0] === 'number') {
        times = recentSolves as number[];
    } else {
        // Filter and sort window solves (exclude DNFs and extreme invalid data)
        const windowSolves = (recentSolves as Solve[])

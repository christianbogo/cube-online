...
exports.evaluateUserGoals = exports.formatStreakDate = exports.formatTimeMs = exports.ALL_TRACKED_KEYBINDS = exports.CATEGORY_METADATA = exports.GOAL_DEFINITIONS = void 0;
const calculations_1 = require("./calculations");
const constants_1 = require("./constants");
// @ts-ignore
const date_fns_1 = require("date-fns");
exports.GOAL_DEFINITIONS = [
...
exports.formatStreakDate = formatStreakDate;
...
function evaluateUserGoals(solves, user, usedKeybinds = [], userStats) {
    let totalSolveTimeMs = 0;
    let totalSolvesCount = 0;
    let maxSolvesInSingleDay = 0;
    let maxDayDate = null;
    let dailySolvesCountMap = new Map();
    let validSolvesPerEvent = {};
    let hasAo5PerEvent = {};
    let hasAo12PerEvent = {};
    let hasAo100PerEvent = {};
    ...
    if (userStats) {
        ...
        totalSolveTimeMs = solves.reduce((acc, curr) => {
            if (curr.penalty === 'DNF' || curr.inspectionPenalty === 'DNF')
                return acc;
            let t = curr.time;
            if (curr.penalty === '+2')
                t += 2000;
            if (curr.inspectionPenalty === '+2')
                t += 2000;
            return acc + t;
        }, 0);
        ...
        const solvesByEvent = {};
        constants_1.SUPPORTED_EVENT_IDS.forEach(id => solvesByEvent[id] = []);
        solves.forEach(s => {
        ...
        constants_1.SUPPORTED_EVENT_IDS.forEach(eventId => {
            ...
            const bestAo5 = (0, calculations_1.calculateBestAverage)(list, 5);
            const bestAo12 = (0, calculations_1.calculateBestAverage)(list, 12);
            const bestAo100 = (0, calculations_1.calculateBestAverage)(list, 100);
    ...
    const eventsWithValidSolve = constants_1.SUPPORTED_EVENT_IDS.filter(id => (validSolvesPerEvent[id] || 0) >= 1).length;
    const eventsWithValidAo5 = constants_1.SUPPORTED_EVENT_IDS.filter(id => hasAo5PerEvent[id]).length;
    const eventsWithValidAo12 = constants_1.SUPPORTED_EVENT_IDS.filter(id => hasAo12PerEvent[id]).length;
    const totalEventsWithAo100 = constants_1.SUPPORTED_EVENT_IDS.filter(id => hasAo100PerEvent[id]).length;
    ...
    const averageableAo12Count = constants_1.AVERAGEABLE_EVENT_IDS.filter(id => hasAo12PerEvent[id]).length;
    const uniqueKeybindsCount = new Set(usedKeybinds).size;
    return exports.GOAL_DEFINITIONS.map(def => {
        let currentValue = 0;
        let displayCurrent = '';
        let displayTarget = '';
...
exports.evaluateUserGoals = evaluateUserGoals;
//# sourceMappingURL=goalsCalculations.js.map
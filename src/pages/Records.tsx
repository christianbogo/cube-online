...
export default function Records() {
    ...
    const { rows, summary } = useMemo(() => {
        ...
        SCRAMBLE_TYPES.forEach(opt => {
            ...
            [single, ao5, ao12, ao50, ao100, ao250, ao1000].forEach(rec => {
                if (rec) {
        ...
        return {
            rows: eventRows,
            summary: {
                totalSolves: totalSolvesCount,
                totalTime: totalPracticeTimeMs,
                activeDays: activeDaysSet.size,
                activeEvents: eventRows.length,
                recentRecords: totalRecentRecords
            }
        };
    }, [relevantSolves]);
    ...
    return (
        <div className="h-full w-full flex flex-col overflow-hidden relative font-sans">
            ...
            <div className="bg-bg-secondary/60 border-b border-border px-6 py-4 shrink-0 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-text-secondary">
                            Comprehensive record report across all puzzle events with session and cross-session best averages.
                        </p>
                    </div>
                </div>

                {/* Color Legend in Top Bar */}
                <div className="hidden lg:flex items-center gap-4 text-[11px] font-mono bg-bg-primary/70 border border-border/60 px-3 py-1.5 rounded-lg">
            ...
            {/* Main Stage & Right Bar Container */}
            <div className="flex-1 flex flex-row overflow-hidden relative">
                {/* Scrollable Records Table Area */}
                <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar flex flex-col gap-6">
                    {/* Overview Stat Cards */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="bg-bg-secondary/40 border border-border/70 rounded-xl p-4 flex flex-col">
                            <span className="text-2xl font-bold text-text-primary font-mono">{summary.totalSolves}</span>
                            <span className="text-[11px] font-medium text-text-secondary uppercase tracking-wider mt-0.5">Total Solves</span>
                        </div>
                        <div className="bg-bg-secondary/40 border border-border/70 rounded-xl p-4 flex flex-col">
                            <span className="text-2xl font-bold text-text-primary font-mono">{formatDuration(summary.totalTime)}</span>
                            <span className="text-[11px] font-medium text-text-secondary uppercase tracking-wider mt-0.5">Total Time</span>
                        </div>
                        <div className="bg-bg-secondary/40 border border-border/70 rounded-xl p-4 flex flex-col">
                            <span className="text-2xl font-bold text-text-primary font-mono">{summary.activeEvents}</span>
                            <span className="text-[11px] font-medium text-text-secondary uppercase tracking-wider mt-0.5">Active Events</span>
                        </div>
                        <div className="bg-bg-secondary/40 border border-border/70 rounded-xl p-4 flex flex-col">
                            <span className="text-2xl font-bold text-amber-400 font-mono">{summary.recentRecords}</span>
                            <span className="text-[11px] font-medium text-text-secondary uppercase tracking-wider mt-0.5">Recent Bests</span>
                        </div>
                    </div>

                    {/* Table Container */}
                    <div className="bg-bg-secondary/40 border border-border/70 rounded-xl overflow-hidden shadow-xs">
                            ...
                            <table className="w-full text-sm text-left border-collapse select-none">
                                <thead>
                                    <tr className="text-xs uppercase bg-bg-secondary text-text-secondary border-b border-border">
                                        <th className="px-4 py-3.5 font-semibold text-left whitespace-nowrap">Event</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Solves</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Mean</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Std</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Time</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Single</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Ao5</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Ao12</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Ao50</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap">Ao100</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap" title="Cross-session eligible">Ao250*</th>
                                        <th className="px-3 py-3.5 font-semibold text-right whitespace-nowrap" title="Cross-session eligible">Ao1000*</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-border/40">
                                    {rows.map(row => (
                                        <tr key={row.type} className="hover:bg-bg-hover/40 transition-colors">
                                            ...
                                            <td className="px-3 py-3 text-right text-text-secondary font-mono text-xs">
                                                {row.count}
                                            </td>
                                            <td className="px-3 py-3 text-right text-text-secondary font-mono text-xs">
                                            ...
                                            <td className="px-3 py-3 text-right text-text-secondary font-mono text-xs whitespace-nowrap">
                                    ...
                                    {rows.length === 0 && (
                                        <tr>
                                            <td colSpan={12} className="px-4 py-12 text-center text-text-secondary italic">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-text-secondary px-1 pb-8">
                        <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-text-primary">*</span>
                            <span>The <strong>ao250</strong> and <strong>ao1000</strong> averages can span from session to session. Single, ao5, ao12, ao50, and ao100 must be achieved within the same session.</span>
                        </div>
                        <div className="text-[11px] text-text-secondary/70 shrink-0">
                            Click any record cell to inspect solve details.
                        </div>
                    </div>
                </div>

                {/* Right Bar Inspector Panel */}
                {selectedRecord && (
...
function RecordCell({ eventName, eventType, detail, isSelected, onClick }: RecordCellProps) {
    if (!detail || detail.value === null) {
        return (
            <td className="px-3 py-3 text-right text-text-secondary/30 font-mono text-xs">
                -
            </td>
        );
    }
    ...
    return (
        <td className="px-1.5 py-1.5 text-right relative">
            <button
                type="button"
                onClick={() => onClick(eventName, eventType, detail)}
                className={`
                    group/cell relative w-full px-2.5 py-1.5 rounded-lg font-mono text-xs text-right transition-all cursor-pointer outline-none
                    ${recencyClass}
                    ${isSelected
            ...
            >
                ...
                {/* Hover Tooltip showing date and time of the first solve */}
                <div className="absolute bottom-full right-1/2 translate-x-1/2 mb-2 hidden group-hover/cell:flex flex-col gap-1 bg-zinc-950 text-white text-[11px] px-3.5 py-2.5 rounded-xl border border-zinc-700 whitespace-nowrap z-50 shadow-2xl pointer-events-none min-w-[200px] text-left animate-in fade-in zoom-in-95">
                    ...
                    <div className="w-2 h-2 bg-zinc-950 border-r border-b border-zinc-700 rotate-45 absolute -bottom-1 right-1/2 translate-x-1/2" />
                </div>
            </button>
        </td>
    );
...
function RecordInspectorSidebar({
    ...
    return (
        <aside
            style={{ width }}
            className="h-full bg-bg-secondary border-l border-border flex flex-col text-text-secondary text-sm overflow-hidden select-none animate-in slide-in-from-right duration-250 shrink-0 relative z-30 font-sans"
        >
            {/* Resize Handle */}
            <div
